...
TextboxList.Autocomplete = new Class({
	...
	initialize: function(textboxlist, options){
		...
		if (Browser.ie) this.textboxlist.setOptions({bitsOptions: {editable: {addOnBlur: false}}});
		...
		if ((this.options.placeholder) || this.options.queryServer) 
	...
	search: function(bit){
		...
		if (this.options.queryRemote){
			if (this.searchValues[search]){
				...
				if (typeOf(data) == 'function') data = data.run([], this);
	...
	addResult: function(r, search){
		var element = new Element('li', {'class': this.prefix + '-result', 'html': [r[3], r[1]].pick()}).store('textboxlist:auto:value', r);
